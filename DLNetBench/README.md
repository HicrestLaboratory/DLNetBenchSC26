...
Before compiling be sure to have the modules loaded. Look at `configs.yaml` to check which module to load for each system.

```bash
./compile.sh <system_name>
```
...
*Note: we can not publish the configurations for the NVL72.*

# General Workflow

Regardless of the target system or supercomputer, the experiment pipeline relies on a four-step process using Python scripts to first tow generate and prepare the experiments. The last two run the baselines and concurrent experiments.

* **`experiments_generator.py`**: This script generates a JSON file containing the theoretical experiment layouts.
* **`expand_experiments.py`**: This script takes the generated JSON and splits concurrent experiments into individual JSON descriptors including system-specific configurations.
* **`run_baselines_placements.py`** OR **`run_baselines_no_placement.py`**: Runs baselines either with or without forced placements.
* **`sbatchman launch -f concurrent.yaml`**: Runs concurrent experiments.


# Commands & Common Flags

## Step 1: Generate the Experiments JSON

This step generates both baselines and concurrent experiments.
...
```bash
python3 experiments_generator.py --help
```

## Step 2: Expand Concurrent Experiments into single JSON files

This step applies splits the concurrent experiments described in the JSON, creating individual JSON descriptors, including system-specific information.
...
```bash
python3 expand_experiments.py --help
```

## Step 3: Run

```bash
# First, make sure that `configs.yaml` and `concurrent.yaml` contain the correct configurations for the target system 
sbatchman set-cluster-name leonardo
...
*Note: the following commands may not be 100% accurate since we have sometimes excluded faulty nodes during the experiments execution.*

## Leonardo

The Leonardo supercomputer has 4 A100 GPUs per node. 
...
This configuration disables powerlaw patterns in favor of uniformly distributed workload patterns.
...
```bash
# Baselines
python run_baselines_no_placement.py --comm-lib nccl --gpu-model GH200 --gpus-per-node 4 --cpus-per-task 72 --system alps_clariden experiments_alps_baselines.json
